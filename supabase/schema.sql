...
-- ===========================================================================
-- events: community events that users add to the map
-- ---------------------------------------------------------------------------
-- Unlike contacts/rights_pages (managed here in SQL), this table is written
-- from the app: any signed-in user can post an event, and only the creator can
-- delete their own. There is no moderation yet — that's a future item, in line
-- with the roadmap's deferred community features.
--
-- Safe to re-run: uses `if not exists` and drops/recreates the policies.
-- ===========================================================================
create table if not exists public.events (
  id          bigint generated by default as identity primary key,
  title       text not null,
  description text,
  category    text not null default 'social',   -- social | sports | worship | workshop | other
  event_date  date not null,
  event_time  text,                              -- optional "HH:MM"
  address     text,                              -- typed address (nullable when a place is picked)
  place_id    text,                              -- optional link to a curated place (e.g. 'lucky-plaza')
...
create policy "users create events"    on public.events for insert with check (created_by = auth.uid());
create policy "creators delete events" on public.events for delete using (created_by = auth.uid());

-- ===========================================================================
-- profiles: one row per user (id = auth uid). Powers personalised greetings
-- and names on org pages / announcements. is_admin gates the announcements
-- admin tools; a trigger stops any client from flipping it — only SQL run as
-- the project owner (auth.uid() is null) can grant admin.
-- ===========================================================================
...
begin
  -- Client requests (auth.uid() set) can never change is_admin.
...
-- ===========================================================================
-- organizations: self-serve org pages (e.g. Sportsatsg). Any signed-in user
-- can create one and becomes its owner; owners can update/delete their own.
-- ===========================================================================
create table if not exists public.organizations (
  id          bigint generated by default as identity primary key,
  name        text not null,
  description text,
  category    text not null default 'community',  -- community | sports | ngo | worship | other
  icon        text,
...
drop policy if exists "orgs are public"    on public.organizations;
...
create policy "users create orgs"  on public.organizations for insert with check (owner_id = auth.uid());
create policy "owners update orgs" on public.organizations for update using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "owners delete orgs" on public.organizations for delete using (owner_id = auth.uid());

-- ===========================================================================
-- announcements: admin-curated feed (legal aid, medical outreach, workshops).
-- Everyone reads; only admins (profiles.is_admin) can write/update/delete.
-- ===========================================================================
create table if not exists public.announcements (
  id          bigint generated by default as identity primary key,
  title       text not null,
  body        text,
  category    text not null default 'community',  -- community | legal | medical | workshop | other
  link        text,
  event_date  date,
...
-- Tag events with an optional org so an org page can list its own events.
alter table public.events add column if not exists organization_id bigint references public.organizations(id);

-- A couple of sample announcements so the feed isn't empty on a fresh backend.
insert into public.announcements (title, body, category)
select 'Free legal aid clinic', 'Walk-in consultations for employment and salary issues, Saturdays at HOME.', 'legal'
where not exists (select 1 from public.announcements);
insert into public.announcements (title, body, category)
select 'Medical outreach this Sunday', 'Free health screenings for migrant domestic workers.', 'medical'
where not exists (select 1 from public.announcements where category = 'medical');
